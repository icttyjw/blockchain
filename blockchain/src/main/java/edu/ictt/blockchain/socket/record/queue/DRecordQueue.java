...
public class DRecordQueue {
	 ...
	 *
	 */
	protected ConcurrentHashMap<String, List<DegreeRecord>> recordConcurrentHashMap=new ConcurrentHashMap<String, List<DegreeRecord>>();
	/*
	 * 记录每类记录需要的数量
	 */
	...
	public void receive(RecordBody recordBody){
		...
		List<DegreeRecord> ls=recordConcurrentHashMap.get(hash);
		/*
		...
		ls.add(recordBody.getDegreeRecord());
		...
		//备份记录
			...
			if(ls.size()==count)
			{
				...
				BlockBody blockbody=new BlockBody(null,ls);
