package edu.ictt.blockchain.socket.upperpbft;
/**
 * pbft算法的各状态
 *
 * 算法流程如下：
 *
 * 当前时间片的锻造者将本地的block打包成Upperblock进行广播。（这一步与之前的算法一致）
 * 收到block序列的委托人节点如果还没有收到过这个Upperblock并且验证（验证过程相对校内的多了对upperBlock的校验和）合法后，就广播一个prepare<h, d, s>消息，其中h为Upperblock的高度，d是Upperblock的摘要，s是本节点签名
 ...
 * 系统在在收到第一个高度为h的Upperblock时，启动一个定时器，当定时到期后，如果还没达成一致，就放弃本次共识。
 * @author wuweifeng wrote on 2018/4/23.
 */
public class UVoteType {
